...
function objectBoundaryLimit(e) { 
	...
	if (obj.getBoundingRect().top < padding ||
		...
		jQuery('#safe-zone-inicator').css({ display: 'block' });
        jQuery('#safe-zone-inicator p').text('Out of safe zone, keep text inside the safe zone');
        setTimeout(() => {

         jQuery('#safe-zone-inicator').css({ display: 'none' });
